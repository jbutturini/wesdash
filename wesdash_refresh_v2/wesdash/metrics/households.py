...
def build_households(acs5: pd.DataFrame, acs1_alloc: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    tables: Dict[str, pd.DataFrame] = {}
    metrics = ["hh_own_children_u18", "hhkids_income_150_plus", "hhkids_income_200_plus"]

    if not acs5.empty:
        tables["households_acs5"] = _longify(acs5, metrics, ["year"])
    if not acs1_alloc.empty:
        tables["households_acs1"] = _longify(acs1_alloc, metrics, ["year"])
